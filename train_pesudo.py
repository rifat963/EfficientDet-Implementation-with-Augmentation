...
def make_predictions_wth_tta(images=None, score_threshold=0.1, model=None):
    with torch.no_grad():
        images = torch.stack(images).cuda().float()
        predictions = []
...
def predict_test(data_loader,net):
    ...
    for images, image_ids in data_loader:
        predictions = make_predictions_wth_tta(images=images,model=net)
...
test_net = get_valnet_file('effdet5-cutmix-augmix-fold3/best-checkpoint-036epoch.bin')
...
for i in range(Round_N):
    ...
    #PATH = 'model.bin'
    #torch.save(new_test_net.state_dict(), PATH)
